...
export function dayTwoTaskA() {
  ...
  for (const { min, max } of ranges) {
    console.log(
      `Processing range: ${min}-${max} which has ${max - min + 1} values`,
    );
    ...
    for (let value = min; value <= max; value++) {
      ...
      for (
        ...
        for (let repeatIdx = 1; repeatIdx < nRepeats; repeatIdx++) {
          const nextSubstr = valueStr.substring(
            repeatIdx * patternLen,
            (repeatIdx + 1) * patternLen,
          );
